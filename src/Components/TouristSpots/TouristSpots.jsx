   ...
   <div className="card grid grid-cols-[repeat(3,0.7fr)] justify-center items-center  p-10 gap-10">
    {
     placeData.slice(0,6).map(place => (
      <div key={place._id} className='flex gap-5 items-center justify-center  rounded-xl  p-5 bg-[#204a5aa8] w-[450px] h-[300px] mb-5' >
       <div>
        <div className="image relative -top-14  border-[4px] border-solid border-[#06283f] rounded-2xl ">
         <img src={place.image} alt="image" className='h-[200px] w-[400px] rounded-xl' />
        </div>
        <div>
         <h1 className='font-bold flex items-center gap-3 text-[14px] font-publicSans mt-1 uppercase text-white' > <GiFireplace /> {place.touristSpotName}</h1>
        </div>
